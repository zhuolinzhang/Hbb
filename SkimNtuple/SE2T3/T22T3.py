...
if args.mode in range(1, 5):
    resultTuple = CheckResults.checkCRABResluts(resultPath, args.t, args.date, args.dataset, args.isData)
    mcList = resultTuple[0]
    dataList = resultTuple[1]
    if args.mode in range(2, 5):
        t3Directory = '/publicfs/cms/user/zhangzhuolin/target_files/{}_{}'.format(args.t, args.date)
        for datasetName in mcList:
            jobName = datasetName + "_" + args.t + "_" + args.date
            CopyFiles.copyFiles(t3Directory, jobName, datasetName, 'mc')
            if args.mode in range (3, 5):
                scriptSavePath = resultPath + '/' + args.t + "_" + args.date + "/" + "mcJobSubmit"
        ...
        for datasetName in dataList:
            ...
            if args.mode in range (3, 5):
                scriptSavePath = resultPath + '/' + args.t + "_" + args.date + "/" + "dataJobSubmit"
