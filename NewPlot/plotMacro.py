...
sys.path.append("./lib")
from Plot import scale_hist, plot_hist, plot_ratio, read_data, plot_blind_ratio
...
root_file_open = open_file('./SRSamples')
skim_file_open = open_file('./SidebandSamples')
...
for fileopenname in root_file_open:
    ...
    print(fileopenname.GetName())
    comparename = get_primary_name(root_file_open.index(fileopenname), './SRSamples')
...
#### Plot skim invariant mass spectrum of DiJet
for fileopenname in skim_file_open:
    for s in skim_selection_list:
        exec('h_RecDiJet_{0}_M = fileopenname.Get("h_RecDiJet_{0}_M")'.format(s))
    comparename = get_primary_name(skim_file_open.index(fileopenname), './SidebandSamples')
...
###### Plot skim distributions (pt, eta, phi)
for fileopenname in skim_file_open:
    for i in hist_mu_list:
        for s in skim_selection_list:
            for k in kinematic_list:
...
for s in mu_selection_list:
    exec('plot_ratio(sig_RecDiMuon_{0}_M, bkg_list_RecDiMuon_{0}_M, "RecDiMuon_{0}_M")'.format(s))
for s in mu_selection_list:
    exec('plot_ratio(sig_RecDiJet_{0}_M, bkg_list_RecDiJet_{0}_M, "RecDiJet_{0}_M")'.format(s))
    exec('plot_blind_ratio(sig_RecDiJet_{0}_M, bkg_list_RecDiJet_{0}_M, sig_RecDiJet_Blind_M, bkg_list_RecDiJet_Blind_M, "RecDiJet_{0}_M")'.format(s))
...
for h in hist_mu_list:
    for s in mu_selection_list:
        for k in kinematic_list:
            ...
            exec('plot_blind_ratio(sig_{0}_{1}_{2}, bkg_list_{0}_{1}_{2}, sig_{0}_Blind_{2}, bkg_list_{0}_Blind_{2}, "{0}_{1}_{2}")'.format(h, s, k))
