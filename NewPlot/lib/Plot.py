...
def read_data(data_list, data_mu_list, data_selection_list, data_kine_list, data_kine_dict):
    for s in data_selection_list:
        exec('h_RecDiMuon_{0}_M = ROOT.TH1F("h_RecDiMuon_{0}_M", "h_RecDiMuon_{0}_M", 60, 75, 105)'.format(s)) # read muon histograms
        exec('h_RecDiJet_{0}_M = ROOT.TH1F("h_RecDiJet_{0}_M", "h_RecDiJet_{0}_M", 75, 50, 200)'.format(s)) # read jet histograms
...
def scale_hist(hist, scale_factor):
    ...
    scale_factor = scale_factor * 0.004536324 # correct to ref. trigger HLT_IsoMu20_v*
...
def stack_fill(hist, stack):
    ...
    color_dict = {'st': ROOT.kAzure + 8, 'tt': ROOT.kAzure + 4, 'dib': ROOT.kSpring + 2, 'qcd': ROOT.kViolet - 4, 'zjets': ROOT.kOrange - 2}
    if re.search('sig', '{}'.format(hist)):
        hist.SetFillColor(ROOT.kRed - 4)
...
def make_blind_legend(legend, hist_sig, hist_bkg_list, total_skim_hist):
    # We must notice that the result of read_data is a list. Here we input data_hist which should be a TH1 histogram to this function.
    bkg_leg_dict = {'st': "Single top", 'tt': "t#bar{t}", 'dib': "ZZ", 'qcd': "QCD", 'zjets': "Z+jets"}
    legend.AddEntry(total_skim_hist, "Total Sideband", "P")
    legend.AddEntry(hist_sig, "ZH(b#bar{b})", "F")
    for i in hist_bkg_list:
        for key, value in bkg_leg_dict.items():
            if re.search(key, '{}'.format(i)):
...
def plot_ratio(hist_sig, hist_bkg_list, stack_name, data_TH1F_list = None, blockSRFlag = False):
    # Plot the stack histogram and Data/MC
    # Sum MC samples and create MC Stat. Err. graphs
    srDict = {"_M": (90, 150)} # Set the range of signal region
    if blockSRFlag == True:
        for key, value in srDict.items():
            if re.search(key, stack_name):
                for i in range(value[0], value[1] + 1, int(hist_sig.GetXaxis().GetBinWidth(0))):
                    binNumber = hist_sig.GetXaxis().FindBin(i)
                    hist_sig.SetBinContent(binNumber, 0)
                    hist_sig.SetBinError(binNumber, 0)
                    for j in hist_bkg_list:
                        j.SetBinContent(binNumber, 0)
                        j.SetBinError(binNumber, 0)
        data_TH1F_list = None
...
def plot_blind_ratio(hist_sig, hist_bkg_list, sideband_hist_sig, sideband_hist_bkg_list, stack_name):
    # Plot the stack histogram of SR and plot the CR/SR
    # Sum sideband MC samples
    sum_sideband_hist = sideband_hist_sig
    sum_sideband_hist.SetStats(0)
    for i in sideband_hist_bkg_list:
        sum_sideband_hist += i
    sum_SR_hist = hist_sig
    for i in hist_bkg_list:
        sum_SR_hist += i
    # Create TCanvas and two TPads to place THStack and Data/MC
    c2 = ROOT.TCanvas("c2", "", 800, 600)
    ...
    hs.SetMaximum(3e3)
    hs.GetYaxis().SetTitleOffset(1)
    hs.GetYaxis().SetTitle("Events / {:.1f}".format(sum_sideband_hist.GetXaxis().GetBinWidth(1)))
    # Plot the MC Stat. Err. and set the style of MC Stat. Err.


    # Plot sideband points
    sum_sideband_hist.SetStats(0) # avoid the statistics box is drawn after plot_hist the data histograms
    sum_sideband_hist.SetLineColor(ROOT.kBlack)
    sum_sideband_hist.SetMarkerStyle(24)
    sum_sideband_hist.Draw("SAME E0")
    '''
    # Check the match of THStack and data are right
    print("------------------")
    print("The data hist name: ", data_hist.GetName())
    ...
    # Set the legend of THStack
    leg = ROOT.TLegend(0.63, 0.70, 0.86, 0.87)
    make_blind_legend(leg, hist_sig, hist_bkg_list, sum_sideband_hist)
    leg.Draw("SAME")
    ...
    lowerPad.SetTopMargin(0.)
    lowerPad.SetBottomMargin(0.3)
    ratio = make_ratio(sum_sideband_hist, sum_SR_hist)
    ratio.GetXaxis().SetLimits(sum_sideband_hist.GetXaxis().GetXmin(), sum_sideband_hist.GetXaxis().GetXmax())
    ratio.SetFillColor(ROOT.kBlack)
    ...
    c2.SaveAs("Stacks/stack_blind_{}.pdf".format(stack_name))