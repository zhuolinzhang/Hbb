...
int ntuple_reducer(TString fileName, TString savePath)
{
    ...
    //{
    // Use RDataFrame to cut the TTree
    TString flatFileName = flatPathName + returnPathOrFileName(fileName, "file");
    ROOT::RDataFrame d(oldTreeName, fileName.View());
    std::cout << "Skim " << flatFileName << std::endl;
    auto branchNames = d.GetColumnNames();
    auto dCutMuon = d.Filter("All(mu1Pt > 25) && All(mu2Pt > 15)", "Muon pt").Filter("All(mu1Tight == 1) && All(mu2Tight == 1)", "Tight muon").Filter("All(mu1Eta < 2.4) && All(mu2Eta < 2.4)", "Muon eta").Filter("All(mu1Iso < 0.4) && All(mu2Iso < 0.4)", "Muon iso.").Filter("All(ZM > 75) && All(ZM < 105)", "Z mass");
    auto dCutJet = dCutMuon.Filter("All(jet1Pt > 20) && All(jet2Pt > 20)", "Jet pt").Filter("All(jet1bTag > 0.4184) && All(jet2bTag > 0.4184)", "deepCSV").Filter("All(jet1ID == 1) && All(jet2ID == 1)", "Jet ID").Filter("All(HiggsM >= 50 && HiggsM <= 200)", "Higgs mass");
    ...
    dCutJet.Snapshot(TString(newTreeName).View(), (savePath + returnPathOrFileName(fileName, "file")).View(), branchNames);
    ...
    //}
    makeFlatTree(savePath + returnPathOrFileName(fileName, "file"), flatPathName, newTreeName, newTreeName);
    return 0;
}
