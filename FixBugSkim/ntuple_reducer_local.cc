...
void makeFlatTree(TString oldDirName, TString oldDirPath, TString newDirPath, const char *oldTreeName, const char *newTreeName)
{
    ...
    for (auto originFile = originFileList.cbegin(); originFile != originFileList.cend(); originFile++)
    {
        ...
        TFile fFlat = TFile((newDirPath + "/" + *originFile).Data(), "RECREATE");
...
int ntuple_reducer()
{
    ...
    auto oldTreeName = "demo/ZHCollection";
    ...
    makeFlatTree(oldDirName, oldPathName, flatPathName, oldPathName, newTreeName);
    ...
    for (auto fileName : flatList)
    {
        ...
        auto d_cut_muon = d.Filter("mu1Pt > 25 && mu2Pt > 15 && mu1Tight == 1 && mu2Tight == 1 && mu1Eta < 2.4 && mu2Eta < 2.4 && mu1Iso < 0.4 && mu2Iso < 0.4 && ZM > 75 && ZM < 105");
        //auto d_cut_jet = d
        //                            .Filter("All(jet1Pt > 20) && All(jet2Pt > 20)");
        //.Filter("All(jet1bTag > 0.4184) && All(jet2bTag > 0.4814)")
        //.Filter("All(jet1ID == 1) && All(jet2ID == 1)")
        //.Filter("All(HiggsM >= 90) && All(HiggsM <= 150)")
        fileName.Remove(0, flatPathName.Length());
        d_cut_muon.Snapshot(TString(newTreeName).View(), (skimPathName + fileName).View(), branchNames);
    }
