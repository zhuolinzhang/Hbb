...
class ZHTest : public edm::one::EDAnalyzer<edm::one::SharedResources> {

   public:

      explicit ZHTest(const edm::ParameterSet&);
      ~ZHTest();
   ...
   private:
      ...
      std::vector<float> *jet1Pt, *jet1Eta, *jet1Phi, *jet1M, *jet1bTag;
      std::vector<float> *jet2Pt, *jet2Eta, *jet2Phi, *jet2M, *jet2bTag;
      std::vector<bool> *mu1Global, *mu1Tight, *mu2Global, *mu2Tight, *jet1ID, *jet2ID;
...
//
ZHTest::ZHTest(const edm::ParameterSet& iConfig) {
...
bool ZHTest::HLTaccept(const edm::Event& iEvent, std::string& HLTPaths) {
...
void ZHTest::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
void ZHTest::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
