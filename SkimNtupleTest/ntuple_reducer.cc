...
void makeFlatTree(TString oldFileName, TString oldDirName, TString oldDirPath, TString newDirPath, const char *oldTreeName, const char *newTreeName)
{
    using namespace std;
    ...
    TFile fFlat = TFile((newDirPath + returnPathOrFileName(oldFileName, "file")).Data(), "RECREATE");
...
int ntuple_reducer(TString fileName, TString savePath)
{
    ...
    //TString skimPathName = savePath + "/Skim";
    //TString muonCutFlowPathName = savePath + "/MuonCutFlow";
    TString jetCutFlowPathName = savePath + "/JetCutFlow";
    checkPath(savePath);
    checkPath(flatPathName);
    //checkPath(skimPathName);
    checkPath(jetCutFlowPathName);
    makeFlatTree(fileName, oldDirName, oldPathName, flatPathName, oldTreeName, newTreeName);
    ...
    ROOT::RDataFrame d(newTreeName, flatFileName.View());
    ...
    cutFlowCalc(fileName, jetCutFlowPathName, dJetCutFlow);
    dCutJet.Snapshot(TString(newTreeName).View(), (savePath + returnPathOrFileName(fileName, "file")).View(), {"ZPt", "ZEta", "ZPhi", "ZM", "HiggsPt", "HiggsEta", "HiggsPhi", "HiggsM"});
    //}
    return 0;
}