...
def getFileFlag(sampleName, taskName, date, mcOrData):
    findFlag = False
    t2Path = 'gsiftp://ccsrm.ihep.ac.cn/dpm/ihep.ac.cn/home/cms/store/user/zhuolinz' # my new T2 path
    if mcOrData == 'mc':
        sampleDirectory = os.popen('gfal-ls -l {0}/{1}'.format(t2Path, sampleName))
    elif mcOrData == 'data':
...
if os.path.exists(resultSavePath): pass
else: os.mkdir(resultSavePath)
noOutputMCList = []
noOutputDataList = []
mcInputList = readLocalList("./SampleList.txt")
dataInputList = readLocalList("./DataList.txt")
mcT2List = []
dataT2List = []
# Get MC list and data list in T2
for i in mcInputList:
    mcFindFlag = getFileFlag(i, taskName, taskFullDate, 'mc')
    if mcFindFlag: mcT2List.append(i)
    else: noOutputMCList.append(i)
for i in dataInputList:
    dataFindFlag = getFileFlag(i, taskName, taskFullDate, 'data')
    if dataFindFlag: dataT2List.append(i)
    else: noOutputDataList.append(i)
