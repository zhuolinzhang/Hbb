...
class ZpeakTrigger : public edm::one::EDAnalyzer<edm::one::SharedResources> {
   ...
   private:
      ...
      // leading muon and next-to-leading muon
      TH1F* h_RecLMuon_Trigger_pt;
      TH1F* h_RecLMuon_MuonMatch_pt;

      TH1F* h_RecNLMuon_Trigger_pt;
      TH1F* h_RecNLMuon_MuonMatch_pt;

      TH1F* h_RecLMuon_Trigger_eta;
      TH1F* h_RecLMuon_MuonMatch_eta;

      TH1F* h_RecNLMuon_Trigger_eta;
      TH1F* h_RecNLMuon_MuonMatch_eta;

      TH1F* h_RecLMuon_Trigger_phi;
      TH1F* h_RecLMuon_MuonMatch_phi;

      TH1F* h_RecNLMuon_Trigger_phi;
      TH1F* h_RecNLMuon_MuonMatch_phi;
...
ZpeakTrigger::ZpeakTrigger(const edm::ParameterSet& iConfig) {
  ...
  // leading muon
  h_RecLMuon_Trigger_pt = fs->make<TH1F>("h_RecLMuon_Trigger_pt",";p_{T};",50,0,500);
  h_RecLMuon_MuonMatch_pt = fs->make<TH1F>("h_RecLMuon_MuonMatch_pt",";p_{T};",50,0,500);

  h_RecLMuon_Trigger_eta = fs->make<TH1F>("h_RecLMuon_Trigger_eta",";#eta;",60,-6,6);
  h_RecLMuon_MuonMatch_eta = fs->make<TH1F>("h_RecLMuon_MuonMatch_eta",";#eta;",60,-6,6);

  h_RecLMuon_Trigger_phi = fs->make<TH1F>("h_RecLMuon_Trigger_phi",";#phi;",40,-4,4);
  h_RecLMuon_MuonMatch_phi = fs->make<TH1F>("h_RecLMuon_MuonMatch_phi",";#phi;",40,-4,4);
  // next-to-leading muon
  h_RecNLMuon_Trigger_pt = fs->make<TH1F>("h_RecNLMuon_Trigger_pt",";p_{T};",50,0,500);
  h_RecNLMuon_MuonMatch_pt = fs->make<TH1F>("h_RecNLMuon_MuonMatch_pt",";p_{T};",50,0,500);

  h_RecNLMuon_Trigger_eta = fs->make<TH1F>("h_RecNLMuon_Trigger_eta",";#eta;",60,-6,6);
  h_RecNLMuon_MuonMatch_eta = fs->make<TH1F>("h_RecNLMuon_MuonMatch_eta",";#eta;",60,-6,6);

  h_RecNLMuon_Trigger_phi = fs->make<TH1F>("h_RecNLMuon_Trigger_phi",";#phi;",40,-4,4);
  h_RecNLMuon_MuonMatch_phi = fs->make<TH1F>("h_RecNLMuon_MuonMatch_phi",";#phi;",40,-4,4);
      ...
      // find mu-
           for (auto mum = muons->cbegin(); mum != muons->cend(); ++mum) {
                ...
                if ( mup->pt() > mum->pt() ){
                  ...
                  LMFlag = 1; // mu+ is the leading muon
                }
                else
                {
                  if ( mup->pt() <= 15.0 ) continue;
                  if ( mum->pt() <= 25.0 ) continue;
                  LMFlag = 0; // mu- is the leading muon
                }
                ...
                if (HLTaccept(iEvent, HLTPath_)) {
                  ...
                  if (LMFlag) {
                    h_RecLMuon_Trigger_pt->Fill(mupRecPt);
                    h_RecLMuon_Trigger_eta->Fill(mupRecEta);
                    h_RecLMuon_Trigger_phi->Fill(mupRecPhi);
                    h_RecNLMuon_Trigger_pt->Fill(mumRecPt);
                    h_RecNLMuon_Trigger_eta->Fill(mumRecEta);
                    h_RecNLMuon_Trigger_phi->Fill(mumRecPhi);
                  }
                  else {
                    h_RecLMuon_Trigger_pt->Fill(mumRecPt);
                    h_RecLMuon_Trigger_eta->Fill(mumRecEta);
                    h_RecLMuon_Trigger_phi->Fill(mumRecPhi);
                    h_RecNLMuon_Trigger_pt->Fill(mupRecPt);
                    h_RecNLMuon_Trigger_eta->Fill(mupRecEta);
                    h_RecNLMuon_Trigger_phi->Fill(mupRecPhi);
                  }
                  if (muon1->triggerObjectMatchByPath(hltName) != nullptr && muon2->triggerObjectMatchByPath(hltName) != nullptr) {
                    ...
                    if (LMFlag) {
                      h_RecLMuon_MuonMatch_pt->Fill(mupRecPt);
                      h_RecLMuon_MuonMatch_eta->Fill(mupRecEta);
                      h_RecLMuon_MuonMatch_phi->Fill(mupRecPhi);
                      h_RecNLMuon_MuonMatch_pt->Fill(mumRecPt);
                      h_RecNLMuon_MuonMatch_eta->Fill(mumRecEta);
                      h_RecNLMuon_MuonMatch_phi->Fill(mumRecPhi);
                    }
                    else {
                      h_RecLMuon_MuonMatch_pt->Fill(mumRecPt);
                      h_RecLMuon_MuonMatch_eta->Fill(mumRecEta);
                      h_RecLMuon_MuonMatch_phi->Fill(mumRecPhi);
                      h_RecNLMuon_MuonMatch_pt->Fill(mupRecPt);
                      h_RecNLMuon_MuonMatch_eta->Fill(mupRecEta);
                      h_RecNLMuon_MuonMatch_phi->Fill(mupRecPhi);
                    }
                  }
